...
def test_forgotmypw_success(client):
...
def test_forgotmypw_failure(client):
    response = client.post(
...
def test_forgotmypw_empty_fields(client):
    response = client.post("/login", data={"username": "", "password": ""})
    assert response.status_code == 401


def test_forgotmypw_short_credentials(client):
    response = client.post(
        "/login", data={"username": "ab", "password": "12345"})
    assert response.status_code == 401


def test_forgotmypw_invalid_credentials(client):
    response = client.post(
